...
def evaluate(model, dataloader, tokenizer,  device, amp=True, recall_k_list=[5]):
    ...
    for batch_images, batch_texts, inds in tqdm(dataloader):
        ...
        batch_texts_tok = tokenizer([text for i, texts in enumerate(batch_texts) for text in texts], truncate=True).to(device)
        ...
        with torch.no_grad(), autocast():
            try:
                ...
                # # Temporary hack for B-cosifyed CLIP-RN50 model
                import clip
                clip_model,_ = clip.load('RN50')
                batch_images_emb = F.normalize(model(batch_images), dim=-1)
                batch_texts_emb = F.normalize(clip_model.encode_text(batch_texts_tok), dim=-1)
