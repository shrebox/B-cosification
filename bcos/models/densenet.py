...
class _DenseLayer(nn.Module):
    def __init__(
        ...
        # Adding act_layer for Bcosification experiments
        self.act1 = act_layer()
    ...
    def bn_function(self, inputs: List[Tensor]) -> Tensor:
        ...
        # Updated for Bcosification experiments
...
class BcosDenseNet(BcosUtilMixin, nn.Module):
    ...
    def __init__(
            ...
            self.features = nn.Sequential(
                OrderedDict(
                    [
                        ...
                        # Not sure if I should add act_layer here!
                    ]
                )
            )
        else:
            self.features = nn.Sequential(
                OrderedDict(
                    [
                        ...
                        ("act0", act_layer()), # Added for Bcosification experiments
        ...
        # Added for Bcosification experiments
        self.act = act_layer() 
    ...
    def forward(self, x: Tensor) -> Tensor:
        # print(f"{x.mean().item()=} {x.min().item()=} {x.max().item()=}")
        # Diff to torchvision: Deleted relu
        features = self.features(x)
        # out = F.relu(features, inplace=True)
        out = self.act(features) # using nn.ReLU() instead of F.relu() in original code
