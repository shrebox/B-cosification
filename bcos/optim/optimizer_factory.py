...
class OptimizerFactory:
    ...
    def create(self, model: torch.nn.Module) -> torch.optim.Optimizer:
        ...
        ## if bcosify is true, then only optimize the b parameters
        ## if bcosify is false, then optimize all parameters
        bcosify = self.args.get("bcosify", None)
        if bcosify is not None:
            if self.args.get("b_opt", None):
                parameters = (p for n, p in model.named_parameters() if n.endswith(".b"))
            else:
                parameters = (p for n, p in model.named_parameters() if not n.endswith(".b"))
            ## remove bcosify and b_opt from args for defining the optimizer without parameter error
            args.pop("bcosify", None)
            args.pop("b_opt", None)
