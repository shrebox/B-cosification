...
class NormedConv2d(nn.Conv2d):
    ...
    def forward(self, in_tensor: Tensor) -> Tensor:
        ...
        if self.use_weight_norm:
            w = self.weight / LA.vector_norm(self.weight, dim=(1, 2, 3), keepdim=True) # [out_channels, in_channels, height, width]
        else:
            w = self.weight
        # For scaling the weights with initial pre-trained weights
        if self.scale is not None and self.use_weight_norm:
            w = self.scale * w
        return self._conv_forward(input=in_tensor, weight=w, bias=self.bias) # Here bias = False not possible as _conv_forward expects bias tensor
