...
class BatchNorm2d(nn.BatchNorm2d):
    ...
    # @classmethod
    # def from_standard_module(cls, standard_module: nn.BatchNorm2d):
        """
        Create a BatchNorm2d from a standard nn.BatchNorm2d module.
        """
        # return cls(
        #     num_features=standard_module.num_features,
        #     eps=standard_module.eps,d
        #     momentum=standard_module.momentum,
        #     affine=standard_module.affine,
        #     track_running_stats=standard_module.track_running_stats,
        # )
...
class DetachableGroupNorm2d(nn.GroupNorm, DetachableModule):
    ...
    @classmethod
    def from_standard_module(cls, standard_module: nn.GroupNorm, model_config: dict):
        """
        Create a GroupNorm from a standard nn.GroupNorm module.
        """
        new_mod = cls(
            num_groups=standard_module.num_groups,
            num_channels=standard_module.num_channels,
            eps=standard_module.eps,
            affine=standard_module.affine,
            # device=standard_module.device,
            # dtype=standard_module.dtype,
        )

        weights = model_config.get("weights", None)
        if weights is not None:
            new_mod.weight.data = standard_module.weight.data
            if standard_module.bias is not None:
                new_mod.bias.data = standard_module.bias.data    
...
class DetachableLayerNorm(nn.LayerNorm, DetachableModule):
    ...
    @classmethod
    def from_standard_module(cls, standard_module: nn.LayerNorm, model_config: dict):
        """
        Create a LayerNorm from a standard nn.LayerNorm module.
        """
        new_mod = cls(
            normalized_shape=standard_module.normalized_shape,
            eps=standard_module.eps,
            elementwise_affine=standard_module.elementwise_affine,
            # device=standard_module.device,
            # dtype=standard_module.dtype,
        )
        ...
        return new_mod
    # if standard_module.elementwise_affine:
    #    new_mod.weight = standard_module.weight
    #   new_mod.bias = standard_module.bias
    # return new_mod
